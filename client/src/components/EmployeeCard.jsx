...
const EmployeeCard = ({ employee, onDelete, onEdit }) => {
    ...
    const handleDelete = async () => {
        if(!confirm(`Are you sure you want to delete ${employee.firstName} ${employee.lastName}? This action can be undone later.`)) {
            return;
        }
        try {
  ...
  return (
    <div className='group relative card card-hover overflow-hidden'>
        ...
        <div className='absolute top-3 left-3 flex gap-2'>
            ...
            {employee.isDeleted && <span className='bg-rose-100 text-rose-600 px-2.5 py-1 text-xs font-semibold rounded-lg shadow-sm'>Deleted</span>}
        </div>
        {!employee.isDeleted && (
